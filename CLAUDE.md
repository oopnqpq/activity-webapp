...
純靜態前端（GitHub Pages）+ Google Apps Script API Layer + Google Services
...
## Google Apps Script 免費方案限制（200人活動關鍵約束）

**每次提出涉及 Apps Script API 的建議前，必須先對照以下限制評估影響。**
...
- 200 人同時觸發 API → 最多 30 個同時跑，其餘立即回傳錯誤
- 前端 Exponential Backoff（最多重試 3 次，延遲 1s → 2s → 4s）是唯一的緩衝機制
- **設計原則：任何新增的 API 呼叫，必須評估「200 人同時觸發」的最壞情境**

### 頁面載入時禁止新增 API 呼叫

頁面載入（`DOMContentLoaded` / IIFE init）時觸發的 API 呼叫，等於所有同時在線的人都會打：

```
❌ 不可：頁面載入時呼叫 lookup / stats / 任何 API
✅ 可以：使用者主動觸發的操作（按鈕、掃碼）才呼叫 API
✅ 可以：先讀 localStorage，只在必要時才呼叫 API
```

**替代方案**：前端狀態優先（localStorage），後端驗證作為最後防線，而非主動同步。

### 每日配額

| 服務 | 免費上限 | 本專案用途 | 風險等級 |
|------|---------|-----------|---------|
| Gmail 寄信 | **100 封/日** | 活動前通知 + M5 PDF 寄送 | ⚠ 高風險 |
| Sheets 讀寫 | 20,000 次/日 | 所有 API 操作 | ✅ 安全 |
| Drive 建檔 | 寬鬆，無明確上限 | Batch.gs 建資料夾 + 上傳 | ✅ 安全 |
| URL Fetch | 20,000 次/日 | 本專案未使用 | ✅ 安全 |

### LockService 行為

- `waitLock(10000)`：等待最多 10 秒取得鎖
- 超過 10 秒仍未取得 → 拋出例外 → 前端收到錯誤 → Backoff 重試
- 同一時間只有 1 個執行可寫入，其他排隊
- **只有寫入操作需要 Lock；純讀取（getValues）不需要但仍佔用並發名額**

### 200 人高峰情境估算

| 操作 | 觸發時機 | 預期並發數 | 評估 |
|------|---------|-----------|------|
| 登入（auth） | 活動開始前 10 分鐘 | ~50人/分鐘 | ✅ 可控 |
| ⑥ 批次送出（batch） | 活動結束前集中 | **最高 ~100人/5分鐘** | ⚠ 需 Backoff 消化 |
| 報到掃碼（scan） | 工作人員手動，1人/次 | <5 | ✅ 安全 |
| 儀表板統計（stats） | 工作人員主動刷新 | <5 | ✅ 安全 |
